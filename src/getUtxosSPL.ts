...
export async function getUtxosSPL({
  publicKey,
  connection,
  encryptionService,
  storage,
  ...
  logger.debug(
    "token name: " + token.name + ", token address" + token.pubkey.toString(),
  );

  try {
    publicKey_ata = await getAssociatedTokenAddress(token.pubkey, publicKey);
    let offsetStr = storage.getItem(
      LSK_FETCH_OFFSET + localstorageKey(publicKey_ata),
    );
    if (offsetStr) {
    ...
    while (true) {
      ...
      if (nonZeroUtxos.length > 0) {
        ...
        for (let i = 0; i < nonZeroUtxos.length; i++) {
          if (!spentFlags[i]) {
            logger.debug(
              `found unspent encrypted_output ${nonZeroEncrypted[i]}`,
            );
  ...
  storage.setItem(
    LSK_ENCRYPTED_OUTPUTS + localstorageKey(publicKey_ata),
...
export function getBalanceFromUtxosSPL(utxos: Utxo[]): {
  ...
  let token = tokens.find(
    (t) => t.pubkey.toString() == utxos[0].mintAddress.toString(),
  );
  if (!token) {
...
async function decrypt_output(
  ...
  try {
    ...
    const decryptedUtxo = await encryptionService.decryptUtxo(encryptedOutput, lightWasm, mintAddress);
    ...
    try {
      ...
      try {
        ...
        try {
          ...
          logger.debug(
            `Could not find commitment account for ${commitment}, using encrypted index: ${res.utxo.index}`,
          );
        }
      }
